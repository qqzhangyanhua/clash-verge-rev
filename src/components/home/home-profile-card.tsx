import {
  CloudUploadOutlined,
  LaunchOutlined,
  StorageOutlined,
} from '@mui/icons-material'
import { Box, Button, Link, LinearProgress, Typography } from '@mui/material'
import { useLockFn } from 'ahooks'
import dayjs from 'dayjs'
import { type ReactNode, useCallback, useMemo, useState } from 'react'
import { useTranslation } from 'react-i18next'
import { useNavigate } from 'react-router'

import { useAppRefreshers } from '@/providers/app-data-context'
import { updateProfile } from '@/services/cmds'
import { showNotice } from '@/services/notice-service'
import { openExternalUrl } from '@/utils/open-external-url'
import parseTraffic from '@/utils/parse-traffic'

const parseUrl = (url?: string) => {
  if (!url) return '-'
  if (url.startsWith('http')) return new URL(url).host
  return 'local'
}

const parseExpire = (expire?: number) => {
  if (!expire) return '-'
  return dayjs(expire * 1000).format('YYYY-MM-DD')
}

const openProfileHome = (url: string) => {
  void openExternalUrl(url).catch(showNotice.error)
}

const ProfileMetaRow = ({
  label,
  children,
  onClick,
}: {
  label: string
  children: ReactNode
  onClick?: () => void
}) => (
  <Box
    className="home-profile__meta-row"
    onClick={onClick}
    sx={onClick ? { cursor: 'pointer' } : undefined}
  >
    <Typography className="home-profile__meta-label">{label}</Typography>
    <Box className="home-profile__meta-value">{children}</Box>
  </Box>
)

const ProfileUsage = ({ current }: { current: IProfileItem }) => {
  const usedTraffic = useMemo(() => {
    if (!current.extra) return 0
    return current.extra.upload + current.extra.download
  }, [current.extra])

  const trafficPercentage = useMemo(() => {
    if (!current.extra || !current.extra.total || current.extra.total <= 0) {
      return 0
    }
    return Math.min(Math.round((usedTraffic / current.extra.total) * 100), 100)
  }, [current.extra, usedTraffic])

  if (!current.extra) return null

  const [used, usedUnit] = parseTraffic(usedTraffic)
  const [total, totalUnit] = parseTraffic(current.extra.total)

  return (
    <Box className="home-profile__usage">
      <Box className="home-profile__usage-figures">
        <span className="home-profile__used">
          {used} {usedUnit}
        </span>
        <span className="home-profile__total">
          / {total} {totalUnit}
        </span>
      </Box>
      <Box className="home-profile__bar-wrap">
        <LinearProgress
          variant="determinate"
          value={trafficPercentage}
          className="home-profile__bar"
        />
        <Typography className="home-profile__pct">
          {trafficPercentage}%
        </Typography>
      </Box>
    </Box>
  )
}

const ProfileDetails = ({
  current,
  onUpdateProfile,
}: {
  current: IProfileItem
  onUpdateProfile: () => void
}) => {
  const { t } = useTranslation()

  return (
    <Box className="home-profile__body">
      <ProfileUsage current={current} />
      <Box className="home-profile__meta">
        {current.url && (
          <ProfileMetaRow label={t('shared.labels.from')}>
            {current.home ? (
              <Link
                component="button"
                onClick={() => current.home && openProfileHome(current.home)}
                sx={{
                  display: 'inline-flex',
                  alignItems: 'center',
                  maxWidth: '100%',
                  fontWeight: 500,
                }}
                title={parseUrl(current.url)}
              >
                <Typography component="span" noWrap>
                  {parseUrl(current.url)}
                </Typography>
                <LaunchOutlined sx={{ ml: 0.5, fontSize: '0.8rem' }} />
              </Link>
            ) : (
              <Typography component="span" noWrap title={parseUrl(current.url)}>
                {parseUrl(current.url)}
              </Typography>
            )}
          </ProfileMetaRow>
        )}
        {current.updated && (
          <ProfileMetaRow
            label={t('shared.labels.updateTime')}
            onClick={onUpdateProfile}
          >
            {dayjs(current.updated * 1000).format('YYYY-MM-DD HH:mm')}
          </ProfileMetaRow>
        )}
        {current.extra && current.extra.expire > 0 && (
          <ProfileMetaRow label={t('shared.labels.expireTime')}>
            {parseExpire(current.extra.expire)}
          </ProfileMetaRow>
        )}
      </Box>
    </Box>
  )
}

const EmptyProfile = ({ onClick }: { onClick: () => void }) => {
  const { t } = useTranslation()

  return (
    <Box className="home-profile__empty" onClick={onClick}>
      <CloudUploadOutlined sx={{ fontSize: 28, color: 'text.secondary' }} />
      <Typography variant="body2">
        {t('profiles.page.actions.import')} {t('profiles.page.title')}
      </Typography>
    </Box>
  )
}

interface HomeProfileCardProps {
  current: IProfileItem | null | undefined
  onProfileUpdated?: () => void
}

export const HomeProfileCard = ({
  current,
  onProfileUpdated,
}: HomeProfileCardProps) => {
  const { t } = useTranslation()
  const navigate = useNavigate()
  const { refreshAll } = useAppRefreshers()
  const [updating, setUpdating] = useState(false)

  const onUpdateProfile = useLockFn(async () => {
    if (!current?.uid || updating) return

    setUpdating(true)
    try {
      await updateProfile(current.uid, current.option)
      onProfileUpdated?.()
      refreshAll()
    } catch (err) {
      showNotice.error(err, 3000)
    } finally {
      setUpdating(false)
    }
  })

  const goToProfiles = useCallback(() => {
    navigate('/profile')
  }, [navigate])

  return (
    <Box className="home-module home-profile">
      <Box className="home-module__header">
        <Typography className="home-module__title" title={current?.name}>
          {current?.name ?? t('profiles.page.title')}
        </Typography>
        {current && (
          <Button
            variant="text"
            size="small"
            onClick={goToProfiles}
            endIcon={<StorageOutlined fontSize="small" />}
          >
            {t('layout.components.navigation.tabs.profiles')}
          </Button>
        )}
      </Box>
      {current ? (
        <ProfileDetails current={current} onUpdateProfile={onUpdateProfile} />
      ) : (
        <EmptyProfile onClick={goToProfiles} />
      )}
    </Box>
  )
}
